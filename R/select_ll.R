...
#' Refine a nested general model from its constrained child
#'
#' Finite-mixture multi-start fits can occasionally leave a more-general model
#' a fraction of a log-likelihood unit below a constrained child.  Because the
#' child solution is feasible in the parent, one additional EM run initialized
#' at that exact solution restores the nesting inequality without changing the
#' model or adding a heuristic decision rule.  The safeguard is used for both
#' observed and bootstrap fits so the statistic remains identically defined.
#'
#' @param data Binary response matrix.
#' @param fit_general,fit_constrained Nested qlfit objects at the same C.
#' @param use_cpp Use the compiled E-step.
#' @return The original general fit, or a higher-likelihood warm-started fit.
#' @keywords internal
.warm_refine_general <- function(data, fit_general, fit_constrained,
                                 use_cpp = TRUE) {
  ...
  type_general <- fit_general$model_type
  type_constrained <- fit_constrained$model_type
  if (is.null(fit_general) || is.null(fit_constrained) ||
      isTRUE(fit_general$polytomous) || isTRUE(fit_constrained$polytomous) ||
      is.null(nested_children[[type_general]]) ||
      !type_constrained %in% nested_children[[type_general]] ||
      fit_general$n_classes != fit_constrained$n_classes ||
      fit_general$loglik >= fit_constrained$loglik - 1e-8) {
    return(fit_general)
  }

  type <- fit_general$model_type
  C <- fit_general$n_classes
  init_p <- fit_constrained$item_probs
  init_w <- fit_constrained$class_probs
  raw <- tryCatch(suppressWarnings(switch(type,
    UN = em_lca(data, C, init_probs = init_p,
                init_class_probs = init_w, use_cpp = use_cpp),
    MON = {
      spec <- specify_constraints(class_monotonicity = TRUE)
      em_constrained(data, C, constraints_spec = spec,
        constraint_fn = build_class_monotonicity_fn(ncol(data), C),
        init_probs = init_p, init_class_probs = init_w,
        method = "pava", use_cpp = use_cpp)
    },
    IIO = {
      ord <- fit_general$item_order
      spec <- specify_constraints(item_ordering = TRUE, item_order = ord)
      em_constrained(data, C, constraints_spec = spec, item_order = ord,
        constraint_fn = build_item_ordering_fn(ncol(data), C, ord),
        init_probs = init_p, init_class_probs = init_w,
        method = "pava", use_cpp = use_cpp)
    },
    DM = {
  ...
  if (is.null(raw) || !is.finite(raw$loglik) ||
      raw$loglik <= fit_general$loglik) return(fit_general)
  out <- fit_general
  out$item_probs <- raw$item_probs
  out$class_probs <- raw$class_probs
  out$posteriors <- raw$posteriors
  out$loglik <- raw$loglik
  out$convergence <- raw$converged
  out$iterations <- raw$iterations
  out$degenerate <- isTRUE(raw$degenerate)
  out
}
...
#'     \item{raw_statistic}{The observed statistic before flooring.}
#'     \item{pre_refinement_statistic}{The observed statistic before any
#'       child-initialized refinement of the general fit.}
...
#'     \item{general_warm_started}{Whether a child-initialized EM refinement
#'       improved the observed general-model fit.}
#'     \item{bootstrap_general_warm_started}{Number of valid bootstrap
#'       replicates whose general fit was improved by the same safeguard.}
#'   }
#'
#' @details
...
ll_equivalence_test <- function(data, fit_constrained, fit_un,
  ...
  fit_un_original_ll <- fit_un$loglik
  lr_pre_refinement <- 2 * (fit_un_original_ll - fit_constrained$loglik)
  fit_un <- .warm_refine_general(data, fit_un, fit_constrained, use_cpp)
  warm_started <- fit_un$loglik > fit_un_original_ll + 1e-8

  # Observed statistic, guarded against optimization noise
  lr_raw <- 2 * (fit_un$loglik - fit_constrained$loglik)
  ...
  if (lr_raw < -0.05) {
    warning("Observed LR statistic is negative (",
            formatC(lr_raw, format = "g", digits = 4), ") for ",
            type_c, " vs ", type_g,
            ": the general model has a lower log-likelihood than the ",
            "constrained model even after a child-initialized refinement. ",
            "Treating the statistic as 0; consider increasing n_starts.")
  }
  lr_obs <- max(0, lr_raw)
  ...
  boot_one <- function(b) {
    ...
    if (is.null(fit_c_star) || is.null(fit_g_star)) {
      return(c(statistic = NA_real_, warm_started = NA_real_))
    }
    fit_g_original_ll <- fit_g_star$loglik
    fit_g_star <- .warm_refine_general(
      boot_data, fit_g_star, fit_c_star, use_cpp)
    fit_g_warmed <- fit_g_star$loglik > fit_g_original_ll + 1e-8

    if (verbose && mc.cores == 1L && b %% 10 == 0) {
      cat("  bootstrap replicate", b, "of", B, "\n")
    }

    c(statistic = max(0, 2 * (fit_g_star$loglik - fit_c_star$loglik)),
      warm_started = as.numeric(fit_g_warmed))
  }

  raw <- par_lapply(seq_len(B), boot_one, mc.cores)
  # coerce any parallel worker failures (try-error, wrong length) to NA
  lr_star_all <- vapply(raw, function(z) {
    if (is.numeric(z) && length(z) >= 1L) z[[1L]] else NA_real_
  }, numeric(1))
  warm_star_all <- vapply(raw, function(z) {
    if (is.numeric(z) && length(z) >= 2L) z[[2L]] else NA_real_
  }, numeric(1))
  valid <- !is.na(lr_star_all)
  lr_star <- lr_star_all
  n_failed <- sum(is.na(lr_star))
  lr_star <- lr_star[valid]
  warm_star <- warm_star_all[valid]
  ...
  structure(
    list(
      statistic = lr_obs,
      raw_statistic = lr_raw,
      pre_refinement_statistic = lr_pre_refinement,
      p_value = p_value,
      B = B,
      B_effective = b_eff,
      n_failed = n_failed,
      ...
      general_warm_started = warm_started,
      bootstrap_general_warm_started = sum(warm_star, na.rm = TRUE)
    ),
    class = "qleqtest"
  )
}
...
#' hours per dataset for long tests and broad class-count grids. `mc.cores`
#' parallelises replicates. Supplying `seed` makes every fit and draw
#' reproducible.
#'
#' @references
...
select_model_ll <- function(data, n_classes, alpha = 0.05, alpha_quant = 0.05,
  ...
  tests <- data.frame(comparison = character(0), statistic = numeric(0),
                      raw_statistic = numeric(0),
                      pre_refinement_statistic = numeric(0),
                      general_warm_started = logical(0),
                      bootstrap_general_warm_started = integer(0),
  ...
  add_test <- function(tests, comparison, test, adequate) {
    rbind(tests, data.frame(
      ...
      raw_statistic = test$raw_statistic,
      pre_refinement_statistic = test$pre_refinement_statistic,
      general_warm_started = isTRUE(test$general_warm_started),
      bootstrap_general_warm_started =
        test$bootstrap_general_warm_started,
  ...
  test_adequate <- function(fit_c, fit_g, label, seed_offset, a = alpha,
    ...
    ok <- t$p_value > a
    # PROTOTYPE degenerate-null retention: the parametric-bootstrap null can be
    # DEGENERATE when the constrained and general models are near-equivalent on
    # the data (e.g. DM vs IIO on doubly-monotone data - both fit identically,
    # null LRs cluster at ~0). A trivially small observed LR then exceeds the
    # whole degenerate null, giving p = 1/(B+1) - a spurious rejection of a
    # negligible effect. When even the null's 95th percentile is a trivially
    # small LR, the bootstrap itself certifies the models indistinguishable, so
    ...
    overridden <- (isTRUE(severity) || degenerate) && ok && t$p_value <= a
    tests <<- add_test(tests, label, t, ok)
    if (overridden && nrow(tests))
      tests$decision[nrow(tests)] <<- if (degenerate)
        "retained (degenerate null: models near-equivalent)"
        else "retained (severity override: no detectable effect)"
    t$label <- label
    ...
    if (lcr_ok) {
      ...
      if (!is.null(fits$RM) && !is.null(lcr_best)) {
        ...
        tests <- rbind(tests, data.frame(
          ...
          raw_statistic = NA_real_, pre_refinement_statistic = NA_real_,
          general_warm_started = NA,
          bootstrap_general_warm_started = NA_integer_,
          p_value = rl$p_value,
